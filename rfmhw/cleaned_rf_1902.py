...
def fit_me(pargs, file_unbal=None, ncpu=10,
    ...
    # Save
    np.savez('data.npy', valid_data=valid_data, train_allvars=train_allvars,
             train_labels=train_labels, labels_valid=labels_valid)
...
def assess(valid_data, train_allvars, train_labels, labels_valid):

    # # Random Forest
    # # Run RF with best model identified by CV 

    # Instantiate model with N decision trees
    rfc = RandomForestClassifier(n_estimators = 10, bootstrap=False , min_samples_split=2, min_samples_leaf=1, max_features="auto", 
                                 random_state = 42, max_depth=80, oob_score=False)
